...
import TableSkeleton from "../../MasterPages/QuestionsList/TableSkeleton";
import Pagination from "../../MasterPages/QuestionsList/Pagination";
import { convertDate } from "../../MasterPages/Quizzes/Quizzes";
import { useNavigate } from "react-router-dom";
import { ResultStudentResponse } from "../../../InterFaces/StudentResultInteface";

export default function ResultStudent() {
  const [results, setResults] = useState<ResultStudentResponse[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [totalCount, setTotalCount] = useState<number>(0);
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [itemPerPage] = useState<number>(8);
  const [paginatedResults, setPaginatedResult] = useState<
  ...
  const handelNavigateToQuizeResult = (result: ResultStudentResponse) => {
  ...
  const getResults = async () => {
    setLoading(true);
    try {
      const response = await apiClient.get<ResultStudentResponse[]>(
        "/quiz/result"
      );
